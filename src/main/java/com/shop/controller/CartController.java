...
public class CartController {
    ...
    ResponseEntity order(@RequestBody @Valid CartItemDto cartItemDto, BindingResult bindingResult, Principal principal) {
        ...
        // 장바구니에 담을 상품 정보를 받는 cartItemDto 객체에 데이터 바인딩시 에러가 있는지 검사
    ...
    // 요청된 자원의 일부를 업데이트할때 PATCH 사용, 장바구니 상품의 수량만 업데이트하기때문에 PathcMapping 사용
    ...
    public @ResponseBody ResponseEntity updateCartItem
        ...
        } else if(!cartService.validdateCartItem(cartItemId, principal.getName())) {
    ...
    public @ResponseBody ResponseEntity deleteCartItem
        ...
        if(!cartService.validdateCartItem(cartItemId, principal.getName())){
            return new ResponseEntity<String>("수정 권한이 없습니다.", HttpStatus.FORBIDDEN);
        }
    ...
    public @ResponseBody ResponseEntity orderCartItem
        ...
        for (CartOrderDto cartOrder : cartOrderDtoList) {
            if(!cartService.validdateCartItem(cartOrder.getCartItemId(), principal.getName())){
